...
print_header() {
    clear_screen
    echo -e "${CYAN}╔════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║      派对制造 Shell 工具箱 v5.1.2      ║${NC}"
    echo -e "${CYAN}╚════════════════════════════════════════╝${NC}"
    echo ""
}
...
func_fetch_nicknames() {
    ...
    echo -e "${BLUE}正在拉取随机昵称...${NC}"
    
    # 请求获取昵称（每批最多10个）
    NICKNAME_RESP=$(curl -s -X GET "https://battlecraft.tuimotuimo.com/battlecraft/account/autoname" \
      -H "Host: battlecraft.tuimotuimo.com" \
      -H "Accept: */*" \
      -H "Accept-Encoding: deflate, gzip" \
      -H "Cookie: $COOKIE" \
      ...
      -H "Content-Type: application/x-www-form-urlencoded" \
      -H "ssn: $SSN" \
      -H "ver: $VER" 2>/dev/null)
    
    # 解析并显示
    echo -e "${GREEN}════════════════ 随机昵称列表 ════════════════${NC}\n"
    
    echo "$NICKNAME_RESP" | python3 -c "
import sys, json
try:
    data = json.load(sys.stdin)
    code = data.get('code', -1)
    ...
    print(f'  📝 共获取到 {len(nicknames)} 个随机昵称：\n')
    
    for idx, name in enumerate(nicknames, 1):
        print(f'  {idx:>2}. {name}')
    
    # 保存到文件
    output_file = '$CONFIG_DIR/nicknames_$(date +%Y%m%d_%H%M%S).txt'
    with open(output_file, 'w', encoding='utf-8') as f:
...
"
    ...
    if [ $? -ne 0 ]; then
        echo -e "${YELLOW}提示：部分解析失败，原始响应如下：${NC}"
        echo "$NICKNAME_RESP"
    fi
