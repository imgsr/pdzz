...
check_and_install_python() {
    # 检查 python3 或 python 是否存在
    if command -v python3 >/dev/null 2>&1; then
        echo -e "${GREEN}✅ Python3 已安装，继续执行脚本${NC}"
        sleep 1
        return 0
    elif command -v python >/dev/null 2>&1; then
        echo -e "${GREEN}✅ Python 已安装，继续执行脚本${NC}"
        sleep 1
        return 0
    fi
    ...
    if [ -d "/data/data/com.termux" ] || command -v pkg >/dev/null 2>&1; then
        ...
        echo -e "${YELLOW}      Termux 用户执行: pkg install python${NC}"
        echo -e "${YELLOW}      Ubuntu/Debian 执行: apt install python3${NC}"
        exit 1
    fi
...
print_header() {
    clear_screen
    echo -e "${CYAN}╔════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║      派对制造 Shell 工具箱 v3.2       ║${NC}"
    echo -e "${CYAN}╚════════════════════════════════════════╝${NC}"
    echo ""
}
