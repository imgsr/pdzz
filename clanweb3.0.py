...
"""
派对制造 - 战队聊天 Web 版 (带IP监控)
修复：切换账号时完整更新，发送消息使用正确的账号
"""
...
def login(account):
    ...
    try:
        ...
        result = resp.json()
        print(f"[登录响应] {account['name']}: {result}")
...
def send_message(account, content):
    ...
    print(f"[发送] {account['name']} 发送: {content}")
    print(f"[发送] Auth: {session_id[:40]}...")
    print(f"[发送] ssn: {account['ssn']}")
    print(f"[发送] cookie: {account['cookie'][:40]}...")
...
def switch_account():
    ...
    if session_id:
        ...
        print(f"[切换账号] ❌ {account['name']} 登录失败")
        return jsonify({
            "success": False,
            "msg": "登录失败"
        })
...
def send():
    ...
    print(f"[发送请求] 当前账号: {account['name']} (key={account_key})")
    
    # 确保登录
    session_id = ensure_logged_in(account)
    if not session_id:
        print(f"[发送] ❌ {account['name']} 登录失败")
